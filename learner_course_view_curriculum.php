...
//		$course_id = 1; 
	...
	$dispSection=mysqli_query($conn, $sel_sql);
...
?>			         
	...
	<h3 style="background:#3c2cd1; text-align:center; padding:8px 0; color:#FFFFFF;">Course Content</h3>
                ...
                <div class="table">
						 ...
						 <?php  
							 while($qfetch = mysqli_fetch_array($dispSection,MYSQLI_ASSOC)) { 
								...
								$dispResources=mysqli_query($conn, $sel_sql2);
								$num_rows = mysqli_num_rows($dispResources);
						...
						<?php 
							if ($num_rows > 0) {
							 while($qfetch = mysqli_fetch_array($dispResources,MYSQLI_ASSOC)) { 
						...
						<?php $free_preview = $qfetch["free_preview"] ;
                              if ($free_preview == "Y") { ?>	
								...
								<div class="tab_column" style=" width:200px;">
									<button type="button" onClick="showResource('<?php echo rawurlencode(base64_encode($qfetch["id"]));?>')" style="color:#fff; background:#428bca; border:none; border-radius:5px;                                    padding:8px 30px; float:right;">View it</button>
								</div>
